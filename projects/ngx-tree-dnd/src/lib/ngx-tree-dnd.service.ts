...
import {FindingResults, TreeConfig, TreeModel} from './models/tree-view.model';
...
export class NgxTreeService {
   ...
   */
    public addNewItem(id, name, icon, visible, parent?) {
        // let pos = 1;
    ...
    public add(id, name, icon, visible, parent?) {
        let pos = 1;
        if (parent && parent.childrens.length !== 0) {
            const parentPrevChildren = parent.childrens.length - 1;
            const newItemPosition = parent.childrens[parentPrevChildren].options.position + 1;
    ...
    */
    ...
    /*
     ...
     Emit onRenameItem Subject.
    */
    public finishRenameItem(name, id, icon, visible) {
        this.elementFinder(this.treeStorage, id);
        // code
        this.findingResults.foundItem.name = name;
        this.findingResults.foundItem.icon = icon;
        this.findingResults.foundItem.visible = visible;
